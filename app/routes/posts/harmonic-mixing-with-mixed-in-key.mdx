...
A track is represented by a **number** and a **letter**. The number represents all possible notes on a piano and the letter represents the scales minor or major. We know there are 12 possible piano notes before we start repeating the same notes.
...
Let's understand what up a fifth means on the Camelot Wheel. It says we can go from **Amin** to **Emin**. Let's use the piano keyboard to visualize this.
